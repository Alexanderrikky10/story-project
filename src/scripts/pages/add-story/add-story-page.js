...
export default class addPage {
  ...
  #setupForm() {
    ...
    this.#form.addEventListener('submit', async (event) => {
      ...
      const photoBlob = this.#takenDocumentations[0]?.blob;
      // Tambahkan validasi ini di sini
      if (!photoBlob) {
        this.storeFailed('Gagal mengambil gambar dari kamera.');
        return;
      }
      let photoFile = null;

      if (photoBlob) {
        photoFile = new File([photoBlob], 'photo.jpg', { type: 'image/jpeg' });
      }
